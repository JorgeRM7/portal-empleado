...
const submitForm = () => {
    ...
    form.put(`/employee/employee-salary-adjustments/${props.Ajuste.id}`, {
        preserveScroll: true,
        onSuccess: () => {
...
watch(() => form.employee_id?.id, (empId) => {
...
const hydrateForm = async (data) => {
    ...
    form.employee_id = employeeOptions.value.find(
        e => e.id === data.employee_id
    ) ?? null

    form.type_salary_adjustment_movement_id = data.type_salary_adjustment_movement_id ?? null
    form.days_period = Number(data.days_period) ?? null
    form.actual_position_id = data.actual_position_id ?? null
    form.actual_department_id = data.actual_department_id ?? null
    form.start_training = data.start_training ?? ''
    form.new_position_id = data.new_position_id ?? null
    form.new_department_id = data.new_department_id ?? null
    form.base_ajuste = data.adjust_base_trial == 1 ? 'prueba' : 'ajuste'
...
onMounted(() => {
    hydrateForm(props.Ajuste);
});

</script>

<template>
    <AppLayout title="Editar ajuste o promoción salarial">

        <div class="card">
            <h2 class="text-2xl font-bold mb-5">
                <i class="pi pi-pencil mr-2 text-warning"></i>
                Editar ajuste o promoción salarial
            </h2>
