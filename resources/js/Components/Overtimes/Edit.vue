...
const isInitialEditLoad = ref(!!props.overtime?.id);
...
const selectedEmployee = ref(
    props.overtime?.employee_id
        ? employeeOptions.value.find((e) => e.id === props.overtime.employee_id)
        : null,
);

const selectedDate = ref(
    props.overtime?.date ? new Date(`${props.overtime.date}T00:00:00`) : null,
);
...
const form = useForm({
    ...
    employee_id: props.overtime?.employee_id || null,
    date: props.overtime?.date || null,
    extemporaneous:
        props.overtime?.untimely == 1 || props.overtime?.untimely === true,
    pay_week_id: props.overtime?.week_number || null,

    time_for_time: props.overtime?.txt == 1 || props.overtime?.txt === true,

    full_shift:
        props.overtime?.complete == 1 || props.overtime?.complete === true,
    double_shift:
        props.overtime?.corrido == 1 || props.overtime?.corrido === true,

    shift_id: props.overtime?.full_shift ? props.overtime?.schedule_id : null,

    external_pay:
        props.overtime?.pay_external == 1 ||
        props.overtime?.pay_external === true,
    plant_id: props.overtime?.external_branch_office_id || null,

    motivo_id: props.overtime?.motivo || null,

    moment: props.overtime?.type || null,
    hours: Number(props.overtime?.hours) || 0,
    double_overtime: JSON.parse(props.overtime?.overtimes).double_overtime || 0,
    triple_overtime: JSON.parse(props.overtime?.overtimes).triple_overtime || 0,

    observations: props.overtime?.comment || "",

    shift_entry_time: props.overtime?.shift_entry_time || null,
    shift_leave_time: props.overtime?.shift_leave_time || null,

    has_error: props.overtime?.has_error || 0,
});

const momentCopy = ref(form.moment);
const hoursCopy = ref(form.hours);
    ...
    if (selectedEmployee.value && selectedDate.value) {
        ...
        try {
            ...
            if (!noData.value) {
                ...
                // PASO 4: Si estamos editando y es la primera carga, NO sobrescribimos las horas
                if (isInitialEditLoad.value) {
                    isInitialEditLoad.value = false; // Apagamos la bandera
                } else {
                    calculateOvertime(); // Solo se calcula si el usuario cambió la fecha o empleado
                }
            } else {
...
const calculateHours = (event) => {
    ...
    // Evita usar "form.overtime" a menos que lo hayas definido en useForm

    // Si event no viene (por ej. llamado sin parámetros), usamos form.hours
    const val = event?.value ?? form.hours;

    if (!form.full_shift) {
        form.double_overtime = val < 9 ? val : 9;
...
// Llamamos la función al montar si estamos editando para traer la tarjeta azul/verde
onMounted(() => {
    if (props.overtime?.id) {
        fetchAttendanceInfo();
    }
});
...
const submitForm = () => {
    ...
    form.put(`/employee/employee-overtimes/overtimes/${form.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            showSuccess();
            sending.value = false;
